...
POPULATION_SIZE: int = 1000      # number of individuals in the population
...
GENERATIONS: int = 10000          # maximum number of generations to evolve
...
class GeneticApproximator:
    ...
    def evaluate_population(self) -> None:
        """Evaluate fitness for the whole population using multithreading."""
        # Use a thread pool to parallelise evaluation.  The pool size is limited
        # by the number of CPU cores to avoid oversubscription.
...
def simplify_node(node: Node) -> str:
    """
    Simplify an expression tree into a human‑readable algebraic string.  This
    function collapses additions and multiplications of variables and constants
    into a canonical form and leaves other operations (abs, trig) untouched.


    RFEWTREWQIHNT MAKE SWITCH CASE UNLESS NUMBA
    RFEWTREWQIHNT MAKE SWITCH CASE UNLESS NUMBA
    RFEWTREWQIHNT MAKE SWITCH CASE UNLESS NUMBA
    RFEWTREWQIHNT MAKE SWITCH CASE UNLESS NUMBA
    RFEWTREWQIHNT MAKE SWITCH CASE UNLESS NUMBA
